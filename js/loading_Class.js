export default function loading_Class(canvas_id, hideCallBack, number = 27) {
  /**判断浏览器是否支持canvas**/
  // try{
  ...
  var image = new Image();
  image.src = require("../images/bd_logo1.jpg");
  ...
  (function init() {
    ...
    for (var i = xIndexStart; i < xIndexEnd; i++) {
      ...
      for (var j = yIndexStart; j < yIndexEnd; j++) {
        ...
        var y = rectWidth * j;
        var _r = Math.pow(
          Math.pow(i - xindexMid, 2) + Math.pow(j - yindexMid, 2),
          0.5
        );
  ...
  function bindMouse() {
    ...
    for (var i = 0; i < logoRectA.length; i++) {
      ...
      var _R = Math.pow(Math.pow(rect._xpos, 2) + Math.pow(rect._ypos, 2), 0.5);
      _R *= 1.2;
      ...
      TweenBlock.insert(
        TweenMax.to(rect, 0.75, { _xpos: _x, _ypos: _y, ease: Back.easeIn }),
        0.01 * (i / 2)
      );
      TweenBlock.insert(
        TweenMax.to(rect, 0.75, {
          _xpos: _xpos,
          _ypos: _ypos,
          ease: Back.easeOut
        }),
  ...
  function showLogo() {
    ...
    TweenBlock.insert(
      TweenMax.staggerTo(
        logoRectA,
        2,
        { angleY: -360, ease: Linear.easeNone },
        0.01
      ),
      0
    );
  ...
  function showBtn() {
    TweenMax.from(btnImg, 1, {
      y: "+60",
      alpha: 0,
      ease: Strong.easeOut,
      delay: 1
    });
    TweenMax.to($("#loading_btn"), 0.75, {
      y: "+=20",
      alpha: 1,
      ease: Strong.easeOut
    });
    ...
    TweenMax.to($("#loading_btn_line_0"), 1, {
      left: 5,
      top: 5,
    ...
    TweenMax.to($("#loading_btn_line_1"), 1, {
      left: 77,
      top: 5,
      ease: Strong.easeInOut,
      repeat: -1,
    ...
    TweenMax.to($("#loading_btn_line_2"), 1, {
      left: 5,
      top: 15,
      ease: Strong.easeInOut,
      repeat: -1,
    ...
    TweenMax.to($("#loading_btn_line_3"), 1, {
      left: 77,
      top: 15,
      ease: Strong.easeInOut,
      repeat: -1,
  ...
  function showPage() {
    ...
    _bgA.sort(function(a, b) {
      // body...
      var _ar = Math.pow(
        Math.pow(a.x / 100 - xindexMid, 2) + Math.pow(a.y / 100 - yindexMid, 2),
        0.5
      );
      var _br = Math.pow(
        Math.pow(b.x / 100 - xindexMid, 2) + Math.pow(b.y / 100 - yindexMid, 2),
        0.5
      );
      return _ar - _br;
    });

    TweenMax.staggerTo(_bgA, 1, { alpha: 0, ease: Linear.easeOut }, 0.005);
    TweenMax.to($("#loading_box"), 1.5, {
      alpha: 0,
      ease: Strong.easeOut,
      delay: 0.5,
      onComplete: clearLoadingBox
    });
  ...
  function render() {
    ...
    for (var i = xIndexStart; i < xIndexEnd; i++) {
      ...
      for (var j = yIndexStart; j < yIndexEnd; j++) {
        ...
        let r = Math.floor(
          rgb * (60 + 40 * rgb / 255) / 100 -
            1124 * Math.pow(1 - coefficient, 2)
        );
        let g = Math.floor(
          rgb * (80 + 20 * rgb / 255) / 100 + 532 * Math.pow(1 - coefficient, 2)
        );
        ...
        ctx.fillStyle =
          "rgba(" + r + "," + g + "," + b + "," + rect.alpha + ")";
        ctx.fillRect(
          rect.x - (2700 - width) / 2,
          rect.y - (1500 - height) / 2,
          rect.width,
          rect.height
        );
      }
      ...
      if (rect.visible) {
        ...
        if (rect.isImage) {
          try {
            ctx.drawImage(
              image,
              Math.floor(rect._xpos + 50),
              Math.floor(rect._ypos + 50),
              4,
              4,
              Math.floor(rect.x),
              Math.floor(rect.y),
              Math.floor(rect.width * rect.scale),
              Math.floor(rect.height * rect.scale)
            );
          } catch (e) {}
        }
  ...
  function rectSprite(_x, _y, _width, _height) {
    ...
    this.colorChange = function() {
      if (Math.random() > 0.99 && this.isStop) {
        this.isStop = false;
        this.isShow = Math.random() > 0.5 ? true : false;
      } else {
        if (this.isShow) this.rgb += 0.1;
        else this.rgb -= 0.1;
